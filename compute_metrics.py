...
ENTAILMENT_THRESHOLD = 0.50   # TODO: tune based on data
...
def get_document_texts(entries: list[dict]) -> list[str]:
    ...
    # TODO: updates column name candidates to match actual final data
    """
    text_col_candidates = [
...
def compute_deepeval_metrics(input_text: str, summary:str, assessment_questions: list[str] | None = None) -> dict:
    ...
    # Could add domain-specific questions once data is known. e.g. Does summary include concerns from different regions?
    ...
    #    print("DeepEval reason:", metric.reason)
        ...
        return {
            ...
            "deepeval_reason": metric.reason, # TODO: add reason back once include_reason=True   
...
def split_summary_into_sentences(summary: str) -> list[str]:
    ...
    Here: use simple sentence splitting as alternative. # TODO: describe in thesis.
    """
...
def compute_entailment_matrix(doc_texts: list[str], summary_sentences: list[str]) -> np.ndarray:
    ...
    We extract the entailment probability (index 1) for each pair.
...
def compute_equal_coverage(
    ...
    Formula: EC(D,S) = (1/K) * sum_k |p(d,s) - p(d,s|a=k)| #TODO: mention reference in thesis (Eq 5 from paper)
    ...
    # Overall coverage probability: p(d,s) = (1/|D||S|) * sum_i sum_j p(d_i, s_j) # TODO: mention in thesis (Eq 4 from paper)
    p_overall = np.sum(entailment_matrix) / (n_docs * n_sents)

    # Per-group coverage probability: p(d,s|a=k) = (1/|D_k||S|) * sum_{d_i in D_k} sum_j p(d_i, s_j) # TODO: mention in thesis (Eq 3 from paper)
...
# -----------------------------------------------------------------------------------------------------------------
# ### Helper for intermediate summaries ###
# def extract_intermediate_texts(pipeline: str, full_output:dict) -> dict:
#     """
#     Returns intermediate summary texts that exist for a give pipeline.
#     Keys map to state field name, values are raw text strings.
#     Returns empty dict for pipelines with no intermediates (baseline 1, grr_no_generators).
#     """
#     if not full_output:
#         return {}
    
#     INTERMEDIATE_KEYS_BY_PIPELINE = {
#         "baseline2":            ["summary"], # pre-reflection summary
#         "grr_full":             ["summary_a", "summary_b", "summary_c"],
#         "grr_no_reviewer":      ["summary_a", "summary_b", "summary_c"],
#         "grr_no_genA":          ["summary_b", "summary_c"],
#         "grr_no_genB":          ["summary_a", "summary_c"],
#         "grr_no_genC":          ["summary_a", "summary_b"],
#         "mbg":                  ["masked_data", "balanced_data"]
#     }

#     keys = INTERMEDIATE_KEYS_BY_PIPELINE.get(pipeline, [])
#     return {
#         ("summary_pre_reflection" if k == "summary" else k): full_output.get(k, "")
#         for k in keys
#         if full_output.get(k, "").strip()
#     }
...
def compute_all_metrics():
    ...
    #    print(f"hedges={hedging_result['hedge_count']} | "
     #         f"freq={hedging_result['hedge_frequency']}"
      #        f"matched={hedging_result['matched_hedges']}")
        ...
        ### For testing skip bc of OpenAI API key
     ...
     #   print(
      #      f"alignment={deepeval_scores['alignment_score']} | "
       #     f"coverage={deepeval_scores['coverage_score']} | "
        ...
        # # Intermediate summary metrics
        # intermediates = extract_intermediate_texts(pipeline, result.get("output"))
        # if intermediates:
        #     metrics["intermediate"] = {}
        #     for label, text in intermediates.items():
        #         print(f"Compuring DeepEval for intemediate '{label}'...")
        #         int_deepeval = compute_deepeval_metrics(input_data, text, assessment_questions=questions)
        ...
        #             labeled_count = sum(1 for l in doc_labels if l is not None)
        #             if labeled_count < 2:
        #                 int_ec[group_col] = {"ec_score": None, "reason": "too few labeled entries"}
        #                 continue
        #             print(f"Computing EC({group_col}) for intermediate '{label}'...")
        #             int_ec[group_col] = compute_equal_coverage(doc_texts, doc_labels, text)
        #             print(f"EC({group_col}) for '{label}' = {int_ec[group_col].get('ec_score')}")
        #         metrics["intermediate"][label] = {
        #             "deepeval":         int_deepeval,
        #             "equal_coverage":   int_ec,
        #         }
        # 
    ...
    #     os.remove(OUTPUT_FILE)   # start fresh; we're rewriting all results
